...
public class CommandThread extends WebSocketServer implements Runnable{
    		...
    		public void run() {
    			...
    			while(true) {
    				...
    				if(ControlQ.isEmpty() != false ) {//Thought isEmpty returns true if empty
	    				...
	    				//System.out.println("Control Out queue empty");
	    			}
    			}
    		}
    ...
    public static String Map(String message) {
    	...
    	for(int i = message.indexOf("]") ;i< message.length(); message = message.substring(i+1), i = message.indexOf("]") ){
    		if(Coordinates.containsKey(message.substring(0,i+1)) == false) {
    			newCords += message.substring(0,i+1);
    			Coordinates.put(message.substring(0,i+1), "PlaceHolder");
    	...
    	//Now message in form x1,y1][x2,y2][x3,y3]/...
...
//    	String message = "0";
//	    	if(ControlOut.remainingCapacity() == 1) {
