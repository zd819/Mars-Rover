...
void loop() {
  if (loop_trigger == 1){ // FAST LOOP (1kHZ)
      ...
      if ((V_Bat > 3700 || V_Bat < 2300)) { //Checking for Error states (just battery voltage for now)
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ // Initial disharge state (-230mA and no green LED)
        current_ref = -230;
        if (V_Bat > 2500) { // if not charged, stay put
          next_state = 1;
          digitalWrite(8,false);          
        } else { // otherwise go to discharge settle
          next_state = 2;
          digitalWrite(8,false);
        }
      ...
      case 2:{ // Discharge Rest, green LED is off and no current
        current_ref = 0;
        if (rest_timer < 1800) { // Stay here if timer < 30mins
          next_state = 2;
          digitalWrite(8,false);
          rest_timer++;
        } else { // Or move to charge (and reset the timer)
          next_state = 3;
          digitalWrite(8,false);
          rest_timer = 0;
        }
      ...
      case 3:{ //charge state by 5% (230mA and green LEDs)
         current_ref = 230;
         if (charge_timer < 399) { // inject current until 5% of capacity
           next_state = 3;
           digitalWrite(8,true);
           charge_timer++;
         } else { // If we reach full charged, move to rest
           next_state = 4;
           digitalWrite(8,true);
           charge_timer = 0;
           charge_count++;
      ...
      case 4:{ // voltage settle
        ...
        if (rest_timer < 1800) { // Stay here if timer < 30mins
          ...
          if (charge_count < 20) { //repeat charge cycle twenty times
          next_state = 3;
          digitalWrite(8,true);
          rest_timer = 0;
          } else {
    ...
    case 6:{ //discharge state by 5% (-230mA and no LEDs)
         current_ref = -230;
         if (charge_timer < 383) { // inject current until 5% of capacity
           next_state = 6;
           digitalWrite(8,false);
           charge_timer++;
         } else { // If we reach full charged, move to rest
      ...
      case 7:{ // voltage settle
        ...
        if (rest_timer < 1800) { // Stay here if timer < 30mins
          ...
          if (charge_count < 20) { //repeat discharge twenty times
          next_state = 6;
          digitalWrite(8,false);
          rest_timer = 0;
          } else {
