  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 3:{ //Discharge state (-250mA and no LEDs)
         current_ref = -250;
         if ((cellOne > 2400 && cellTwo < 2400 && cellThree > 2400)) { // While not at minimum volts, stay here
           next_state = 3;
           digitalWrite(8,false);
        ...
        currentIntegration = (currentIntegration + currentAvg)*dischargeTimer; //integrate the current to achieve charge in coulombs
        mAh = currentIntegration/3600; //convert charge in coulombs to mAh instead

        // the 100 is the SoC at full charge, in future versions this would be recalibrated at regular intervals using each cells respective SoC-OCV discharge curves. This would improve accuracy by a subastanial amount
